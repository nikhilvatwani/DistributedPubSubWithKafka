    ...
    try {
      ...
      if (id_user && id_topic) {
        let topicAll = await Topics.find()
          var mapFR = new Map();
          topicAll.map((cur) => {
            mapFR.set(String(cur._id),cur.topic_name);
          })
          //console.log("all topic = "+ topicAll)
        Topics.findOne({topic_name: id_topic}, function(err, document) {
            if(err){
                res.status(500).send({
                      error: "Topic not available",
             ...
             unsub = Messages.find({ id_topic: document._id },
                    function(err, document) {
                        result = []
                        document.map((cur)=>{
                            cur.id_topic = topic_name
                        })
                        document.map((curr)=>{
                            console.log(curr)
                            result.push(curr)
                        })
                        console.log("seng forward request message from broker1")
...
module.exports.notify = async (req, res) => {
  ...
  try {
    if (id_user) {
      ...
      console.log("send notify message from broker1")
      var response = new Object();
      response.body = {"id_user" : id_user, "message":result}
      stream.write(Buffer.from(JSON.stringify(response)))
      res.status(200).json({
